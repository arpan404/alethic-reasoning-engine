"""Bulk operations service functions."""
    ...
    async with AsyncSessionLocal() as session:
        ...
        for app_id in application_ids:
            try:
                ...
                if not application:
                    failed.append({"application_id": app_id, "error": "Not found"})
                    continue
                
                if application.status == "rejected":
                    failed.append({"application_id": app_id, "error": "Already rejected"})
                    continue
                ...
                failed.append({"application_id": app_id, "error": str(e)})
        
        await session.commit()
    
    if send_notification and successful:
        try:
            from workers.tasks import queue_bulk_rejection_emails
            await queue_bulk_rejection_emails(successful, reason)
        except Exception as e:
