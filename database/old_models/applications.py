...
class Applications(Base):
    ...
    job_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("jobs.id"), nullable=False
    )
    ...
    # AI screening scores
    ai_screening_score: Mapped[float | None] = mapped_column(
        Numeric(5, 4)
    )  # 0.00 to 1.00
    ai_screening_status: Mapped[str | None] = mapped_column(
        String(50)
    )  # pending, completed, failed
    ai_recommendation: Mapped[str | None] = mapped_column(
        String(50)
    )  # strong_match, match, weak_match, no_match

    # Source tracking
    source: Mapped[str | None] = mapped_column(
        String(100)
    )  # job_board, referral, direct, linkedin
    applied_at: Mapped[str] = mapped_column(String(100), nullable=False)
