...
from database.models.organizations import (
    Organization,
    OrganizationUsers,
...
def setup_relationships():
    ...
    if not hasattr(Organization, "users"):
        ...
        Organization.files = relationship(
            "File", foreign_keys="File.organization_id", back_populates="organization"
        )
    ...
    if not hasattr(File, "organization"):
        File.organization = relationship(
            "Organization", foreign_keys="File.organization_id", back_populates="files"
        )
        File.uploader = relationship("User", foreign_keys="File.uploaded_by")

    # Job relationships
    if not hasattr(Job, "organization"):
        Job.organization = relationship("Organization", back_populates="jobs")
        Job.department = relationship("JobDepartment", back_populates="jobs")
        ...
        Job.applications = relationship("Applications", back_populates="job")

    # JobDepartment relationships
    if not hasattr(JobDepartment, "jobs"):
        JobDepartment.jobs = relationship("Job", back_populates="department")

    # JobRequirement relationships
    if not hasattr(JobRequirement, "job"):
        JobRequirement.job = relationship("Job", back_populates="requirements")

    # JobLocation relationships
    if not hasattr(JobLocation, "job"):
        JobLocation.job = relationship("Job", back_populates="locations")
    ...
    if not hasattr(Candidate, "education"):
        ...
        Candidate.applications = relationship(
            "Applications", back_populates="candidate"
        )
    ...
    if not hasattr(CandidateSkill, "candidate"):
        CandidateSkill.candidate = relationship("Candidate", back_populates="skills")
    ...
    if not hasattr(Applications, "job"):
        Applications.job = relationship("Job", back_populates="applications")
