...
class AIAssessmentGrading(Base):
    ...
    confidence_score: Mapped[float | None] = mapped_column(Numeric(5, 4))  # 0.00 to 1.00
...
class AdaptiveTestingSession(Base):
    ...
    candidate_assessment_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("candidate_assessments.id"), unique=True, nullable=False, index=True
    )
    
    # Adaptive parameters
    starting_difficulty: Mapped[str] = mapped_column(String(20), default="medium", nullable=False)
    ...
    difficulty_adjustments: Mapped[list | None] = mapped_column(JSON)  # History of adjustments
    
    # AI decision log
    ai_decisions: Mapped[list | None] = mapped_column(JSON)  # Track AI decisions for transparency
...
class AssessmentProctoring(Base, SecurityMixin):
    ...
    multiple_faces_detected: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
...
class ScreeningWorkflow(Base):
    ...
    # Auto-advance rules
    auto_advance_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    auto_advance_threshold: Mapped[float | None] = mapped_column(Numeric(5, 4))
    
    # Auto-reject rules
    auto_reject_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class ScreeningWorkflowExecution(Base):
    ...
    final_decision: Mapped[str | None] = mapped_column(String(50))  # advance, reject, manual_review
